...
const Shoe = () => {
    ...
    return (
        <div className="p-4 w-full flex flex-col md:flex-row items-center justify-center gap-12 px-0 md:px-6 lg:px-20">
            <motion.div initial={{opacity: 0, x:-50}} whileInView={{opacity: 1, x:0}} transition={{duration: 0.6}} className="w-full p-2 flex items-center justify-center">
                <img src={shoe.image} alt={shoe.name} className="w-80 rounded-lg mt-2" />
            </motion.div>
            <motion.div initial={{opacity: 0, x:50}} whileInView={{opacity: 1, x:0}} transition={{duration: 0.6}} className="w-full p-2 flex flex-col gap-2">
